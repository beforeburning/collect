...
// 正则替换内容
const regularUrl = (link: string, classCount: number, pageCount: number) => link.replace('#####', `${classCount}`).replace('*****', `${pageCount}`);

// 正则替换内容
const regularUrlJK = (link: string, ID) => link.replace('#####', `${ID}`);

// 去除收尾空格 以空格分割 拿到时间
const timeCharacter = (time: string) => time.replace(/(^\s*)|(\s*$)/g, '').split(' ')[0];

// 有来 下一页 获取ID
const urlNextId = (link: string) => link ? <number>parseInt(link.split('.html')[0].split('_').pop()) : null;

// 有来 url容错 +1
const youlaiNextErr = (link: string) => {
...
// 健康一线 封面图分割出时间
const jiankangyixianTime = (link: string, url: string) => {
...
// 健康一线 url分割出时间
const jiankangyixianLinkTime = (link: string) => {
    let timeArr = link.split('vodjk.com/')[1].split('/');
